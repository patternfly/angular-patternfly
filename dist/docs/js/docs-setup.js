NG_DOCS={
  ...
  "pages": [
    ...
    },
    {
      ...
      "keywords": "$scope absolutely accumsan active add add_content addclass adipiscing adipscing aeque aliquam allow amet angular api application aria-expanded aria-haspopup aria-labelledby bar based brand brand-alt brandalt brandsrc btn btn-primary button callback card-pf card-pf-accented card-pf-aggregate-status card-pf-aggregate-status-count card-pf-aggregate-status-mini card-pf-aggregate-status-notification card-pf-aggregate-status-notifications card-pf-body card-pf-heading card-pf-title caret change children class classes click col-md-3 col-sm-6 col-xs-12 collapse conjunction container container-cards-pf container-fluid container-pf-nav-pf-vertical container-pf-nav-pf-vertical-with-sub-menus content contentcontainer controller convenire copiosae corrumpit current dashboard data-placement data-toggle default delenit delicatissimi demo deps deserunt detracto directive display displayed dolor dropdown dropdown-menu dropdown-toggle dropdownmenu1 dropdownmenu2 element example-info-text example-page-container exerci exit fa fa-dashboard fa-gamepad fa-graduation-cap fa-paper-plane fa-rebel fa-shield fa-space-shuttle false faux-layout filters flag function gb getelementbyid gubergren handlenavigateclick has-sub-menus hassubmenus header height help hidden hiddenicons hide-show hide-vertical-nav hideverticalnav href html icon iconclass icons ignored ignoremobile image includedcontent innerhtml intellegam invoked ipsum item itemclickcallback items js layout-pf layout-pf-fixed level link list list-style-type location lorem main marked mediocrem menu menus method mhz mobile mock module nav nav-item-iconic navbar navbar-iconic navbar-nav navbar-right navigate navigate-callback navigatecallback navigation navigationitems ng-controller nonumy novum onclick option options parts patrioque patternfly pericula persistent persistentsecondary pf-vertical-navigation pficon pficon-add-circle-o pficon-cluster pficon-error-circle-o pficon-help pficon-image pficon-ok pficon-user pficon-warning-triangle-o pinnable-menus pinning primary principes product progress progress-bar progress-bar-danger progress-bar-remaining progress-bar-success progress-bar-warning progress-container progress-description progress-description-left progress-label-right progress-label-top-right progressbar quaeque qualisque queryselector quotas ready recteque remaining removeclass return role row row-cards-pf script secondary selector set sets showverticalnav sort sr-only src structure style suavitate submenu submenus supports tertiary text tha title tooltip top total transcluded true update-active-items-on-click updateactiveitemsonclick urbanitas username utilized utils utroque vertical verticalnavlayout vertnavcontroller views vituperatoribus width works"
    },
    {
      "section": "api",
