NG_DOCS={
  ...
  "pages": [
    ...
    },
    {
      ...
      "keywords": "$q $scope $timeout achieve action action-buttons actionbuttons actionfn actions addnewcomponenttodom address angular angular-datatables api array asc associate basic bedrock bird boston bourbon brown button buttons california called changes chase checkbox checkbox-inline checkboxes checked cheryl city class col-md-12 colorado column columns componenet component config configuration controller cross default demo denver determining dilorenzo dinosaur directive disabled display displayed displays dropdown dt-options dtoptions dtoptionsbuilder east edwards elm empty empty-state-config emptystateconfig eventtext example-container exampletableview exists false field flintstone font-weight form-group frank fred function getitems github goal goals good grouped guide handlecheckboxchange header helplink helps hill holly hollywood html http icon impression info init invoke io isdisabled isseparator item itemfield items itemsavailable itemsservice jim john js jump kim label lane linda list list-style-type livingston main marie massachusetts massacusetts matching mcgovern menu menu-actions menuactions mike module nashville ng-change ng-controller ng-if ng-model nichols norfolk notify null oak object objects oncheckboxchange optional order padding-top pat pattern patternfly pennsylvania perform performaction pfexample pficon-warning-triangle-o pftableview pftoolbar pittsburgh property provide reject ren rendering resolve return row rows second selected selection selectionmatchprop service settimeout settings showcheckboxes showcomponent simple smith specific street style switcher table tablectrl tableview taylor tennessee text thomas title toolbar toolbars tooltip tree true type unavailable url urllabel user users uuid var view views virginia washingstone york"
    },
    {
      "section": "api",
