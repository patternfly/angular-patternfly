NG_DOCS={
  ...
  "pages": [
    ...
    },
    {
      ...
      "keywords": "$scope actual alternative amount angular api arc auto-format automatic bar block blue card chart chart-data chart-footer chart-title chartctrl chartdata charts class color colors controller cpu custom data data1 data2 data3 data4 data5 data6 denotes dependent directive disk displayed displays donut error example examples exist fill footer-label-format footer1 footer2 footerlabelwidth function gb html indicator inline js label label-title labels layout layoutinline layoutmultidata layouts left-hand list-style-type mb memory mhz module multi-color multi-data multidata ng-controller number object openstack orange patternfly percent percentage pf-utilization-bar-chart ram reached red regular rendering representing right-hand script side single smaller standard storage style subdata subtitle support surpassed tb three threshold threshold-error threshold-warning thresholds title title1 title2 title3 title4 title5 title6 titlelabelwidth total type units units1 units2 units3 units4 units5 usage utilization valid values warning width"
    },
    {
      "section": "api",
