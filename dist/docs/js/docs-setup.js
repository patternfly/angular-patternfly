NG_DOCS={
  ...
  "pages": [
    ...
    },
    {
      ...
      "keywords": "$scope add angular api application bind c3 c3chartdefaults chart chartconfig chartctrl chartid charts columns config configuration container controller data default dependency directive donut form function getdefaultdonutconfig groups html js library mhz module ng-controller note null options order patternfly pf-c3-chart role script style total type width wrapping"
    },
    {
      "section": "api",
    ...
    },
    {
      ...
      "keywords": "$scope angular api block block-padding blockpadding blocks call ce0000 chart chart-data-available chart-title chartctrl chartdataavailable charts charttitle class click-action clickaction clicked col-md-5 colors console controller correspond d4f0fa data dataavailable default defaults directive displayed ec7a08 example-heatmap-container f00 f9d67a false flag function heapmap heatmap heatmap-color-pattern heatmapcolorpattern height hightest hover html js kubernetes labels legend legend-labels legendlabels list-style-type log m8y max-block-size maxblocksize maximum measurement message module ng-controller node openshift overriding padding patternfly pf-heatmap pixels provider range rendering row script show-legend showlegend showlegends size style threshold thresholds title titlealt titlenodata tooltip total true units utilization values var"
    },
    {
      "section": "api",
      ...
      "keywords": "$scope amount angular api area array axis bind c3 chart chart-data chart-height chartctrl chartdata chartheight chartid charts class col-md-12 color config configuration container contents controller current d-- data dates day default defaultconfig determining directive display element examplesparkline1 examplesparkline2 examplesparkline3 examplesparkline4 full function functions gb generation gettime height html http js label label-title legend list list-style-type markup mhz module ng-controller options org override overrides patternfly percent percentage percentageconfig percentages pf-sparkline-chart point points properties push rendering return row script set setting settings showing showxaxis showyaxis size sparkline style today tooltip tooltipfn tooltiptype total type unit units usageperday usageperdayconfig valid valueperday valueperdayconfig values var width xdata ydata"
    },
    {
      "section": "api",
      ...
      "keywords": "$scope actual amount angular api array axis c3 card chart chart-data chartctrl chartdata chartheight chartid charts class col-md-12 combines compact compactconfig config configuration controller d-- data dataavailable dates days default demo directive disk display element exampletrendschartcompact exampletrendschartinline exampletrendschartlarge exampletrendschartsmall false flag format frame full function gb gettime height html http inline inlineconfig js label label-title large largeconfig latest layout list list-style-type mhz minutes module network ng-controller nodata number options org override patternfly percentage pf-trends-chart pfsparklinechart point points push rendering representing row script settings showing showxaxis showyaxis sizes small smallconfig sparkline style time timeframe title titles today tooltiptype total trend trendlabel trends true unique unit units utilization values valuetype var virtual width xdata ydata"
    },
    {
      "section": "api",
    ...
    },
    {
      ...
      "keywords": "$scope add amount angular api array availablecenterlabel availablechart availabledonutconfig availableexamplesparkline availablesparklineconfig axis c3 center center-label chart chart-data chartctrl chartdata chartid charts class col-md-12 combines config configuration contents controller d-- data dataavailable dates default directive display displays donut donut-config donutcenterlabel donutconfig element error false flag full function gb gettime height html http js label label-title list list-style-type memory mhz module ng-controller nodata nonecenterlabel nonechart nonedonutconfig noneexamplesparkline nonesparklineconfig number options org override parameter patternfly pct percent percentcenterlabel percentchart percentdonutconfig percentexamplesparkline percentsparklineconfig pf-utilization-trend-chart pfdonutchart pfdonutpctchart pfsparklinechart points push rendering representing row script settings showing showsparklinexaxis showsparklineyaxis sparkline sparkline-chart-height sparkline-config sparklinechartheight sparklineconfig specifics specifies style thresholds title today tooltiptype total trend true unit units usage usedcenterlabel usedchart useddonutconfig usedexamplesparkline usedsparklineconfig utilization values var warning width xdata ydata"
    },
    {
      "section": "api",
    ...
    },
    {
      ...
      "keywords": "$scope allowed angular api button close closed controller danger default delay directive disappear element enabled false function header headers html info ispersistent js main manually message module ng-controller notification notificationdemoctrl notifications patternfly persistent persistentmessage pf-notification-header pf-notification-message pf-notification-persistent pf-notification-type pfnotificationheader pfnotificationmessage pfnotificationpersistent pfnotificationtype script success text timeout true type types visual warning won"
    },
    {
      "section": "api",
    ...
    },
    {
      ...
      "keywords": "$scope action action-buttons actionbuttons actionfn actions address allow angular api applicable array based beam bedrock boston bourbon boxes button buttons california call called change changes checkbox checkdisabled checkdisableditem checked city class click clicked clicking col-md-12 colorado config configuration controller cross current dblclick default denver deps determine determining dinosaur directive disable disabled display displayed dlbclick double dropdown east edwards elm enable-button-for-item-fn enablebuttonforitemfn enabled event eventtext example-container example-list-view examplelistview false flintstone font-weight frank fred function grouped handle handlecheckboxchange handleclick handledblclick handleselect handleselectionchange hide holly hollywood html invoke isdisabled isseparator isvisible item items jim john js jump length linda list list-group-item-heading list-group-item-text list-style-type list-view-container list-view-pf-additional-info list-view-pf-additional-info-item list-view-pf-description livingston main marie massachusetts matching mcgovern menu menu-actions menuactions module multiple multiselect nashville ng-controller nichols norfolk notify oak oncheckboxchange onclick ondblclick onselect onselectionchange optional pat patternfly pennsylvania perform performaction pf-list-view pittsburgh placeholder property remains rendering return row rows script second selected selecteditems selection selectionmatchprop selections selectitems selecttype separator set settings showdisabled showselectbox smith street style tennessee thomas title tooltip true unavailable update update-menu-action-for-item-fn updatemenuactionforitemfn utils uuid var view viewctrl views virginia washingstone york"
    }
  ],
  "apis": {
    "api": true
