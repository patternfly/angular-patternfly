NG_DOCS={
  ...
  "pages": [
    ...
    },
    {
      ...
      "keywords": "$ctrl $q $scope $timeout achieve action action-buttons actionbuttons actionfn actions addnewcomponenttodom address angular angular-datatables api array asc associate attribute basic bedrock bird boston bourbon brown button buttons california call callback called cell cells changes chase checkbox checkbox-inline checkboxes checked cheryl city class clicked col-md-12 colactionfn colorado column columns componenet component config configuration controller cross default demo denver determining dilorenzo dinosaur directive disabled display displayed displays dropdown dt-options dtoptions dtoptionsbuilder east edwards elm embedded empty empty-state-config emptystateconfig equal error eventtext example-container exampletableview exists false field flintstone font-weight form-group frank fred function getitems github goal goals good grouped guide handlecheckboxchange handlecolaction header helplink helps hill holly hollywood href html htmltemplate http icon impression info init invoke io isdisabled isseparator item itemfield itemfld items itemsavailable itemsservice jim john js jump key kim label lane linda list list-style-type livingston main marie massachusetts massacusetts matching mcgovern menu menu-actions menuactions mike module name_template nashville ng ng-change ng-click ng-controller ng-if ng-model nichols norfolk notify null oak object objects oncheckboxchange onnameclick optional optionaly order padding-top paramenter passed pat pattern patternfly pennsylvania perform performaction pfexample pficon pficon-error-circle-o pficon-ok pficon-warning-triangle-o pftableview pftoolbar pittsburgh property provide reject ren render rendering resolve return row rows second selected selection selectionmatchprop service settimeout settings showcheckboxes showcomponent simple smith specific status status_template street style switcher table tablectrl tableview taylor template tennessee text thomas title toolbar toolbars tooltip tree true type unavailable url urllabel user users uuid var view views virginia warning washingstone york"
    },
    {
      "section": "api",
