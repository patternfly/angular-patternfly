NG_DOCS={
  ...
  "pages": [
    ...
    {
      "section": "api",
      "id": "patternfly.canvas.directive:pfCanvas",
      "shortName": "pfCanvas",
      "type": "directive",
      "moduleName": "patternfly.canvas",
      "shortDescription": "Directive for core operations and rendering of a canvas. Does not work in IE 11 or lower because they do not support",
      "keywords": "$broadcast $on $scope action actions additional addnode advanced angular api appear args array background background-image background-repeat backgroundcolor browsers bundle canvas canvas-demo-container canvasdemoctrl characters chart chart-data-model chart-view-model chartdatamodel chartviewmodel checkbox checkbox-inline chrome class col-md-12 color connect connection connections connector connectorindex connectors container controller copy core data define defined defines deleted deleteselected demo deselectall dest destination details directive edit endpoints equals event eventtext example-container fa fa-share-alt fa-tag false family fff firefox flag font font-weight fontawesome fontcontent fontfamily fontsize foreignobject form-group function getselectednodes helper hidden hide-connectors hideconnectors html icon iconclass identifier image increment indicating initialized input inputconnectors isonlyonenodeselected js key latest left list-style-type lower main margin-left methods mode module modules moved ms-edge netapp network newnode ng-click ng-controller ng-model node nodeactionclicked nodeactionconnect nodeactionedit nodeactions nodeactiontag nodeid nodes note nuage numnewnodes object objects openshift operations output padding-top passed patternfly patternflyicons-webfont pf pficon-edit pficon-service place plan png read-only readonly rectangle rendering repeat rows script selectall selected side single size source specification storage style subsequent support svg tag tested toolbar top true type types unicode url user valid validconnectiontypes var vmware width work x-coordinate y-coordinate zoomin zoomout"
    },
    {
      "section": "api",
      "id": "patternfly.canvas.directive:pfCanvasEditor",
    ...
    },
    {
      ...
      "keywords": "$scope address allitems allow angular api apple applied appliedfilters applyfilters april array august bar bedrock birth birthmonth boulevard box call car case category cfme-row-column changes cincinatti class col-md-12 col-md-2 col-md-3 col-md-4 col-md-7 comparisons complex-select component config configuration controller corolla crosstrek current currently december default delimiter dinosaur directive display east echo elm entered events-label example-container examplefilter false february field fields filter filterable filtercategories filtercategoriesplaceholder filterchange filterconfig filterdelimiter filtermultiselect filters filterstext filtertype filtervalues flintstone foreach frank fred function green html impreza include input insensitive insensitve item items january john js judy july june length list list-style-type livingston main march match matches matchesfilter matchesfilters model module month multiple ng-controller ng-repeat norfolk november null number objects october ohio onfilterchange optional outback pat patternfly pennsylvania pittsburgh placeholder prius push regexp resultscount return returned row rows script second select selection separating september serve settings single smith space street style subaru subaru-impreza subaru-outback text thomas title toyota toyota-echo toyota-prius true type unique valid values var viewctrl virginia washingstone whoes york"
    },
    {
      "section": "api",
    ...
    },
    {
      ...
      "keywords": "$scope angular api application applicationlaunchercontroller badge button class classes click col-xs-12 component controller custom default directive disable display displayed dropdown fa fa-dashboard false flag function grid hidden-icons hiddenicons href html icon iconclass icons indicator interface is-disabled is-list isdisabled islist item items js label launch launcher link list list-style-type lorem menu module nav navbar navbar-collapse navbar-left navbar-pf navigate navigation navigationitems ng-controller patternfly pficon-build pficon-domain pficon-home pficon-storage-domain pre-demo-text recteque rendering row script style suavitate title tooltip user"
    },
    {
      "section": "api",
    ...
    },
    {
      ...
      "keywords": "$ctrl $q $scope $timeout achieve action action-buttons actionbuttons actionfn actions addnewcomponenttodom address angular angular-datatables api array asc associate attribute basic bedrock bird boston bourbon brown button buttons california call callback called cell cells changes chase checkbox checkbox-inline checkboxes checked cheryl city class clicked col-md-12 colactionfn colorado column columns componenet component config configuration controller cross default demo denver determining dilorenzo dinosaur directive disabled display displayed displays dropdown dt-options dtoptions dtoptionsbuilder east edwards elm embedded empty empty-state-action-buttons empty-state-config emptystateactionbuttons emptystateconfig equal error eventtext example-container exampletableview exists false field flintstone font-weight form-group frank fred function getitems github goal goals good grouped guide handlecheckboxchange handlecolaction header helplink helps hill holly hollywood href html htmltemplate http icon impression info informational init invoke io isdisabled isseparator item itemfield itemfld items itemsavailable itemsservice jim john js jump key kim label lane linda list list-style-type livingston main marie massachusetts massacusetts matching mcgovern menu menu-actions menuactions mike module name_template nashville ng ng-change ng-click ng-controller ng-if ng-model nichols norfolk notify null oak object objects oncheckboxchange onnameclick optional optionaly order padding-top paragraph paramenter passed pat pattern patternfly pennsylvania perform performaction performemptystateaction pfexample pficon pficon-error-circle-o pficon-ok pficon-warning-triangle-o pftableview pftoolbar pittsburgh property provide reject ren render rendering resolve return row rows second secondary selected selection selectionmatchprop service settimeout settings showcheckboxes showcomponent simple smith specific status status_template street style switcher table tablectrl tableview taylor template tennessee text thomas title toolbar toolbars tooltip tree true type unavailable url urllabel user users uuid var view views virginia warning washingstone york"
    },
    {
      "section": "api",
      ...
      "keywords": "$ctrl $filter $scope $timeout absolute achieve action action-buttons actionbuttons actionfn actions actions-label actionsconfig actionsinclude actionstext addnewcomponenttodom address address_template age allitems allowed angular angular-datatables api apple appliedfilters applyfilters april array associate attribute august backward bedrock birth birthmonth boulevard bourbon brown button buttons california call callback called cell cells changes checkbox checkbox-inline checkboxes chris cincinatti class clicked col-md-12 colactionfn colorado column columns comment compatibility componenet component config configuration configuring content controller controls december default demo denver determining dinosaur directive directives disabled display displayed displaylength displays dom don dropdown dt-options dtoptions dtoptionsbuilder east ellipsis elm embedded empty empty-state-config emptystateactionbuttons emptystateconfig equal error example example-container exampletoolbar exists false february field fields filter filterchange filterconfig filters filtertype filtervalues flex flintstone foreach form-group frank fred full function github goal goals good green grouped guide handlecheckboxchange handlecolaction header helplink helps henry hidden holly hollywood href html htmltemplate http icon impression increments indicate info informational initial inline-block invoke io isdisabled isseparator istableview item itemfield itemfld items itemsavailable jane january jim john js judy july jump june key label larry left length linda list list-style-type livingston liz longer main march match matches matchesfilter matchesfilters matching max-width mcgovern menu menu-actions menuactions min-width module modules month monthvals moreactions name_template nashville neccesary ng ng-change ng-click ng-controller ng-if ng-model ngdoc nichols norfolk note notify november nowrap null number oak object objects october ohio oncheckboxchange onfilterchange onnameclick optional optionaly order overflow-x overflow-y padding page-config pageconfig pagenumber pagesize pagesizeincrements pagination paginationtype paragraph paramenter parameters parseint passed pat pattern patternfly pennsylvania perform performaction performtableaction pf pfexample pficon pficon-error-circle-o pficon-ok pficon-warning-triangle-o pftoolbar pfviewutils pittsburgh placeholder position prefered primaryactions properties property provide push relative remove render required resultscount return row rows script second select selected selectedcount selecteditems selection selectionmatchprop september settings showcheckboxes showcomponent size smith sortconfig specific status status_template stree street style switch switcher tabbable table tableactionbuttons tableconfig tablemenuactions tableview tabpane template tennessee text text-bottom text-overflow thing thomas time title todo toolbar toolbaractionsconfig toolbarconfig toolbars tooltip top totalcount tp true truncate-text-container truncate-text-ellipsis type unavailable updateitemsavailable url urllabel user users uuid values var ve vertical-align view viewctrl views virginia visibility visible warning washingstone wasted white-space width wilma york"
    },
    {
      "section": "api",
    ...
    },
    {
      ...
      "keywords": "$scope achieve action actionfn actions address allow angular api apple applicable bedrock block border border-top boulevard boxes buttons call called card cards change changes checkbox checkbox-inline checkdisabled checkdisableditem checked cincinatti city class click clicked clicking col-md-12 component config configuration controller current data dblclick default deps determine determining dinosaur directive disabled display displays dlbclick double east elm empty empty-state-action-buttons empty-state-config emptystateactionbuttons emptystateconfig event events-label eventtext examplecardview exists false flintstone form form-group frank fred function goal goals good green guide handle handlecheckboxchange handleclick handledblclick handleselect handleselectionchange height helplink helps hr html icon impression info informational item items itemsavailable john js judy label length list-style-type livingston main margin matching module multiple multiselect ng-change ng-controller ng-disabled ng-model norfolk notify objects ohio oncheckboxchange onclick ondblclick onselect onselectionchange optional padding paragraph pat pattern patternfly pennsylvania perform performemptystateaction pfexample pficon-warning-triangle-o pittsburgh property provide radio radio-inline remains rendering return role row rows script second secondary select selected selecteditems selection selectionmatchprop selections selectitems selecttype set settings showdisabled showselectbox smith solid specific street style thomas title true type updateselectiontype url urllabel user users utils uuid var view viewctrl views virginia washingstone width york"
    },
    {
      "section": "api",
